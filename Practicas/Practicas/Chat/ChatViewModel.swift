...
class ChatViewModel: ObservableObject {
    ...
    func fetchData(){
        /*
        self.messages = [
            AppMessage(id: "1", text: "Hola, ¿cómo estas?", role: .user, createdAt: Date()),
    ...
    func sendMessage() async throws{ //envia el mensaje al modelo
        print(apiKey)
        var newMessage = AppMessage(id: UUID().uuidString, text: messageText, role: .user) //var
        do{
        ...
        if messages.isEmpty{ //se genera una nueva mensaje
            setupNewChat()
        }
    ...
    private func generateResponse(for message: AppMessage) async throws { //recibe la respuesta del modelo de OpenIA
